...
- `src/main.ts` owns UI state, input handling, helpers, and persistence.
...
The UI keeps a runtime state with board, score, best score, helper mode, selected cells, and undo history. Pure engine functions never mutate the incoming board, which makes moves easy to test and safe to undo.
